...
spark = session.builder.appName('Heighest salary from each deparment').getOrCreate()
...
schema = ['EmployeeId', 'Name', 'Department', 'Salary', 'JoiningDate']

employee_df = spark.createDataFrame(employee_data, schema)
partition_window = wn.partitionBy('Department').orderBy(desc('Salary'))
ranked_df = employee_df.withColumn('Salary_rnk', dense_rank().over(partition_window))

result_df = ranked_df.filter(col('Salary_rnk') == 1)
